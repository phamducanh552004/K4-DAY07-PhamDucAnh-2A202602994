...
python bench.py --strategy fixed | Out-File -Encoding utf8 ket_qua_benchmark_fixed.txt
python bench.py --strategy recursive | Out-File -Encoding utf8 ket_qua_benchmark_recursive.txt
```
...
## Đối chiếu checkpoint trước khi nộp

| Checkpoint | Trạng thái | Bằng chứng |
|---|:---:|---|
| CP1 — Chạy baseline trước khi code | ✅ | Baseline ghi nhận 11 test pass, 31 test fail do các hàm còn `NotImplementedError` |
| CP2 — Chốt corpus và metadata | ✅ | 8/8 file Markdown khớp `sources.csv`; đủ `doc_id`, `source_url`, `audience`, `department`, `document_version`, `language` |
| CP3 — Hoàn thiện chunking | ✅ | `FixedSizeChunker`, `SentenceChunker`, `RecursiveChunker` và comparator đã được cài đặt |
| CP4 — Hoàn thiện vector store và agent | ✅ | Toàn bộ 42/42 unit tests pass |
| CP5 — Chốt benchmark | ✅ | 5 query có gold answer, `must_contain`, thang 0/1/2 và câu 1 dùng filter `audience=student` |
| CP6 — So sánh chiến lược bằng LocalEmbedder | ⚠️ Cần khóa bằng chứng | Đã có raw output FixedSize 6/10. Cần chạy lại và lưu raw output cho Recursive/Heading/Sentence; riêng số chunk Recursive trong báo cáo cũ là 83 nhưng code hiện tại đếm 76 |
| CP7 — Báo cáo, demo và nộp repo | ⚠️ Chưa xong | Báo cáo đã điền; còn demo, raw output Recursive, kiểm tra Git và push commit cuối |

**Quy tắc chốt số liệu:** chỉ sửa dấu ⚠️ thành ✅ sau khi `python bench.py --all` chạy bằng LocalEmbedder trên chính commit chuẩn bị nộp. Số chunk và điểm trong bảng so sánh phải khớp nguyên văn với output đã lưu; không dùng kết quả MockEmbedder để kết luận chất lượng semantic.
...
| Thuyết trình (Demo) | 4 / 5 *(tự đánh giá theo kịch bản đã chuẩn bị; điểm chính thức phụ thuộc phần trình bày)* |
...
- **Thuyết trình 4/5** — đã có kịch bản nêu bài toán, demo A/B metadata filter, so sánh bốn chiến lược và phân tích failure case. Chưa tự chấm tuyệt đối vì chất lượng phần hỏi đáp và thời lượng chỉ có thể xác nhận khi trình bày thực tế.
